...
//	HAL_GPIO_WritePin(GPIOC,GPIO_PIN_7,GPIO_PIN_RESET);//电机使能正输入(GPIO_PIN_RESET:开始，GPIO_PIN_SET:停止）
//	osDelay(10);
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,GPIO_PIN_RESET);//电机方向控制(GPIO_PIN_RESET:顺时针，GPIO_PIN_SET:逆时针）
	osDelay(10);
	HAL_TIM_Base_Start_IT(&htim8);//开启TIM8定时器中断（20KHZ，周期50us）

//	TIM8->CCR1 = 50;//修改占空比
//	__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, 50);//修改占空比
	__HAL_TIM_SET_AUTORELOAD(&htim8,1000);//修改自动重装载值
...
//		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET) //硬触发
  }
}
