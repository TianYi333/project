...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)//GPIO中断回调函数
{
	if(GPIO_Pin==IN0_Pin)
	{
		...
		if(Extreme_position_flag==0&&Sports_status_flag==1)
		{
...
//	else if(htim->Instance==TIM3)//间隔定时器中断，是时候计算速度了（5ms处理一次）
//	{
//		Encoder_Processing_flag=1;
//	}
	...
	{			
		Encoder_Processing_flag=1;
		if(HAL_GPIO_ReadPin(TIM8_CH2_GPIO_Port,TIM8_CH2_Pin)==GPIO_PIN_RESET)//正转
		{
			Motor_pulsec_ount++;
		}
		else if(HAL_GPIO_ReadPin(TIM8_CH2_GPIO_Port,TIM8_CH2_Pin)==GPIO_PIN_SET)//反转 
		{
			Motor_pulsec_ount--;
		}
	}	
