			...
			if(Sports_status_flag==1)
			{
				...
				if(number>=100)
				{
					...
					while(number_i>number)
					{
						...
						else if(4000>number_i&&number_i>=400)
						{
...
//	HAL_TIM_Base_Start_IT(&htim3);                    //开启100us定时器中断
  for(;;)
  {
		if(Encoder_Processing_flag==1)
		{
			...
			else
			{
				...
				if(Zero_regression_flag==1&&Absolute_position_flag==1)//到达绝对定位位置停止运动且之前进行过回零且接收到绝对位置
				{
...
void Extreme_position_Drive(void *argument)//极限位置控制
{
	uint16_t number_a=10000;
	uint16_t AUTORELOAD=20000;
  for(;;)
  {
		osDelay(1);
		if(Extreme_position_flag==1&&Sports_status_flag==1)
		{
			...
			AUTORELOAD=(One_Circle_Distance*1000000*10)/(zero_return_speed_value*Subtraction/100);			
			while(number_a>AUTORELOAD)
			{
				__HAL_TIM_SET_AUTORELOAD(&htim8,number_a-1);//修改自动重装载值
				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
				if(number_a>=4000)
				{
				...
				else if(4000>number_a&&number_a>=400)
				{
