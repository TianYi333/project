...
#define Pulse_Filtering 50   		//脉冲滤波us
...
void Pulse_acquisition(void *argument)
{
  /* USER CODE BEGIN Pulse_acquisition */
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);//配置DIR引脚低电平，电平转换器B->A
  ...
  for(;;)
  {
    osDelay(1);
		if(IC_DONE_FLAG == 1)	// 如果完成一次高电平捕获
		{
			...
			if(Pulse_Cycle<Pulse_Filtering*1.2&&Pulse_Cycle>Pulse_Filtering*0.8)	//滤波
			{
		...
		if(START_FLAG==1)//判断是否接受到开始捕获命令
		{
			...
			{	
				if(Pulse_count!=0)
				{
					if(Pulse_frame_rate<Pulse_standard_frame_rate)//判断帧数
					{
						...
						Pulse_count=0;	//当前段脉冲上升沿数量清0
						TJCPrintf("gather.t7.txt=\"%d\"",0);	//显示高电平时间清0
						TJCPrintf("gather.t9.txt=\"%d\"",0);	//显示一段脉冲高电平数量清0
			...
			else
			{
				if(Pulse_Cycle_Send_array[0]==0x55&&Pulse_Cycle_Send_array[5]==0xff)
				{
					...
					TJCPrintf("gather.t7.txt=\"%d\"",Pulse_Cycle_Send);	//显示高电平时间
					Pulse_count_total++;	//脉冲高电平总数
					Pulse_count++;	//一帧脉冲高电平数量	
					TJCPrintf("gather.t9.txt=\"%d\"",Pulse_count);	//显示一段脉冲高电平数量
					TJCPrintf("gather.t11.txt=\"%d\"",Pulse_count_total);	//显示脉冲高电平总数量	
				}
			}	
		}
  }
