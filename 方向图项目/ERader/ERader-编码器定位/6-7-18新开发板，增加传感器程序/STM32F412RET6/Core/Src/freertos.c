...
uint8_t Hard_Trigger_flag=0;//硬触发启停标志位	
/* USER CODE END PTD */
				...
				case 0x10://写多个保持寄存器(请求：MBAP(7位) 功能码 起始地址H 起始地址L 寄存器数量H 寄存器数量L 字节长度 寄存器值)
					if(Sports_status_flag==0)//未开始运动
					{
						...
						else if(Drive_control_array[8]==0x01)//判断起始地址
						{
							if(Drive_control_array[9]==0x2C)//判断起始地址——起始硬触发位置
							{
								Hard_Trigger_Number=(Drive_control_array[13]<<24|Drive_control_array[14]<<16|Drive_control_array[15]<<8|Drive_control_array[16]);//大端传输数据处理——起始硬触发位置0.001mm
								send(1,send_buff,12);//向客户端发送	
								memset(send_buff,0,13);														
							}
							else if(Drive_control_array[9]==0x2E)//判断起始地址——硬触发间隔
...
//				if(Position_upload>=Target_Position)//到达绝对定位位置停止运动
//				{
//					HAL_TIM_PWM_Stop_IT(&htim8,TIM_CHANNEL_1);//关闭PWM输出
...
{	
  ...
  {
		osDelay(1);
		if(Hard_Trigger_flag==1&&Position_upload>=Hard_Trigger_Position&&Sports_status_flag==1)//硬触发条件(硬触发启动，到达硬触发位置，运动状态)
		{
			if(Hard_Trigger_Current_Number<Hard_Trigger_Number)//硬触发次数判定(硬触发启动，到达硬触发位置，运动状态)
			{		
