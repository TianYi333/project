...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)//GPIO中断回调函数
{
	...
	else if(GPIO_Pin==IN1_Pin)//原点
	{
		...
		__HAL_TIM_SetCounter(&htim2, 10000);//校验位置
...
{
	HAL_GPIO_WritePin(CH2_GPIO_Port,CH2_Pin,enable);//PC7-电机使能正输入(GPIO_PIN_RESET:开始，GPIO_PIN_SET:停止）
	osDelay(1);	
	HAL_GPIO_WritePin(CH2N_GPIO_Port,CH2N_Pin,direction);//PB0-电机方向控制(GPIO_PIN_RESET:顺时针，GPIO_PIN_SET:逆时针）
	osDelay(1);	
	__HAL_TIM_SET_AUTORELOAD(&htim8,number);//修改自动重装载值
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  /* USER CODE BEGIN Callback 1 */
	else if(htim->Instance==TIM3)//间隔定时器中断，是时候计算速度了（1ms处理一次）
	{
		Encoder_Processing_flag=1;
	}
	...
	else if(htim->Instance==TIM13)//硬触发定时1us一次
	{
