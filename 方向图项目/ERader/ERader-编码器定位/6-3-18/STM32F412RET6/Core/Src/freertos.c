...
void Drive(void *argument)//控制驱动器
{
	...
	uint8_t status=0;	
	uint32_t running_speed_value=0;//运行速度
	uint32_t zero_return_speed_value=100;//回零速度
				...
				case 0x03://读保持寄存器
					...
					else if(Drive_control_array[8]==0x00)//判断起始地址
					{
						...
						else if(Data_Bytes==4)
						{
							...
							{		
								send_buff[8]=Data_Bytes;//接下来数据的字节长度
								send_buff[9]=(running_speed_value>>8)&0xFF;//寄存器1H-数据
								send_buff[10]=running_speed_value&0xFF;//寄存器1L-数据
								send_buff[11]=0X00;//寄存器2H-数据
								send_buff[12]=0X00;//寄存器2L-数据
								send(1,send_buff,13);//向客户端发送	
								memset(send_buff,0,13);															
...
//								}
				...
				case 0x10://写多个保持寄存器(请求：MBAP(7位) 功能码 起始地址H 起始地址L 寄存器数量H 寄存器数量L 字节长度 寄存器值)
					...
					if(Drive_control_array[8]==0x00)//判断起始地址
					{
						if(Drive_control_array[9]==0x64)//判断起始地址——回零速度
						{
							zero_return_speed_value=(Drive_control_array[13]<<24|Drive_control_array[14]<<16|Drive_control_array[15]<<8|Drive_control_array[16]);//大端传输数据处理——回零速度
							send(1,send_buff,12);//向客户端发送	
							memset(send_buff,0,13);																
						}
						else if(Drive_control_array[9]==0x68)//判断起始地址——回零蠕变速度
						{
							zero_return_creep_speed_value=(Drive_control_array[13]<<24|Drive_control_array[14]<<16|Drive_control_array[15]<<8|Drive_control_array[16]);//大端传输数据处理——回零蠕变速度
							send(1,send_buff,12);//向客户端发送	
							memset(send_buff,0,13);									
						}
						else if(Drive_control_array[9]==0x78)//判断起始地址——运行速度
						{
							running_speed_value=(Drive_control_array[13]<<24|Drive_control_array[14]<<16|Drive_control_array[15]<<8|Drive_control_array[16]);//大端传输数据处理——运行速度
							send(1,send_buff,12);//向客户端发送	
							memset(send_buff,0,13);		
						}							
						else if(Drive_control_array[9]==0x7A)//判断起始地址——相对定位距离
						...
						{
							Target_Position=(Drive_control_array[13]<<24|Drive_control_array[14]<<16|Drive_control_array[15]<<8|Drive_control_array[16]);//大端传输数据处理——绝对定位位置
							send(1,send_buff,12);//向客户端发送	
							memset(send_buff,0,13);														
						}		
						else if(Drive_control_array[9]==0x7E)//判断起始地址——设备最大运行行程
