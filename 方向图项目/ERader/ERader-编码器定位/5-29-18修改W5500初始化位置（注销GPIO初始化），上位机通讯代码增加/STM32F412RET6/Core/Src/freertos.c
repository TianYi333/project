...
//								}
							...
							case 0x01://原点回归/停止
								if(Drive_control_array[10]==0xFF)//回归
								{
									...
									HAL_GPIO_WritePin(CH2N_GPIO_Port,CH2N_Pin,GPIO_PIN_SET);//PB0-电机方向控制(GPIO_PIN_RESET:顺时针，GPIO_PIN_SET:逆时针）
									osDelay(1);	
									__HAL_TIM_SET_AUTORELOAD(&htim8,zero_return_speed_value);//修改自动重装载值
//									__HAL_TIM_PRESCALER(&htim8,running_speed_value);//修改预分频值
//									TIM8->CCR1 = running_speed_value;//修改占空比
//									__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, running_speed_value);//修改占空比
							...
							case 0x02://正转点动/停止
								if(Drive_control_array[10]==0xFF)//开始
								{
									HAL_GPIO_WritePin(CH2_GPIO_Port,CH2_Pin,GPIO_PIN_RESET);//PC7-电机使能正输入(GPIO_PIN_RESET:开始，GPIO_PIN_SET:停止）
									osDelay(1);	
									HAL_GPIO_WritePin(CH2N_GPIO_Port,CH2N_Pin,GPIO_PIN_RESET);//PB0-电机方向控制(GPIO_PIN_RESET:顺时针，GPIO_PIN_SET:逆时针）
									osDelay(1);	
									__HAL_TIM_SET_AUTORELOAD(&htim8,running_speed_value);//修改自动重装载值
//									__HAL_TIM_PRESCALER(&htim8,running_speed_value);//修改预分频值
//									TIM8->CCR1 = running_speed_value;//修改占空比
//									__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, running_speed_value);//修改占空比
									HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
