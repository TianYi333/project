					...
					if(array_Drive_control[8]==0x20)//判断起始地址
					{
						switch(array_Drive_control[9])//判断起始地址
						{
							...
							case 0x01://原点回归/停止
								...
								array_Send_buff[5]=0X05;//数据长度
								array_Send_buff[8]=0X01;//接下来数据的字节长度
								if(flag_Motion_state==0&&REACH)//回零
								{
					...
					if(array_Drive_control[8]==0x20)//判断起始地址
					{
						switch(array_Drive_control[9])//判断起始地址
						{
							case 0x01://原点回归/停止
								...
								{
									if(AWAY&&flag_Motion_state==0)//不在原点且停止运动
									{
...
//									HAL_TIM_PWM_Stop_IT(&htim8,TIM_CHANNEL_1);//关闭PWM输出
							...
							case 0x05://绝对运动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									array_Send_buff[11]=0X00;
									send(1,array_Send_buff,12);//向客户端发送	
									if(flag_Motion_state==0&&flag_Extreme_position==0)
									{
...
//		osDelay(1);	
		...
		{	
			...
			osDelay(1);	
			while(variable_Current_speed>number_v&&flag_Motion_state==1)
			{
				number_v++;
...
{	
  ...
  {
		osDelay(1);
		if(flag_Hard_trigger_start==1&&variable_Position_upload>=variable_Hard_trigger_position&&flag_Motion_state==1)//硬触发条件(硬触发启动，到达硬触发位置，运动状态)
		{
...
void Motor_deceleration_stops(void *argument)//电机减速停止
{
  ...
  for(;;)
  {
		...
		{	
			...
			while(number_v>5&&flag_Motion_state==1)
			{
				number_v--;
		...
		{	
			...
			number_v=variable_Current_speed;
			while(number_v>1&&flag_Motion_state==1)
			{
				number_v--;
				number_Reloading=5000/number_v;
				__HAL_TIM_SET_AUTORELOAD(&htim8,number_Reloading-1);//修改自动重装载值
