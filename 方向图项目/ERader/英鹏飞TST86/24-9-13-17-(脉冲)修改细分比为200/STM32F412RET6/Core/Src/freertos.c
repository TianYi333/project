						...
						switch(array_Drive_control[9])//判断起始地址
						{
								...
								if(array_Drive_control[10]==0xFF)//回归
								{
									if(AWAY&&CEASE)//不在原点且停止运动
									{
										...
										if(HAL_GPIO_ReadPin(IN0_GPIO_Port,IN0_Pin)==GPIO_PIN_RESET)//在头部-正向移动
										{
											...
											array_Motor_drive_send[1]=((One_Circle_Distance*5000000)/(variable_Zero_return_speed_value*1))>>8&0xff;
											array_Motor_drive_send[2]=(One_Circle_Distance*5000000)/(variable_Zero_return_speed_value*1)&0xff;
							...
							case 0x02://正转点动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									memset(array_Send_buff,0,13);	
									array_Motor_drive_send[0]=GPIO_PIN_RESET;
									array_Motor_drive_send[1]=((One_Circle_Distance*5000000)/(variable_Running_speed_value*1))>>8&0xff;//自动重装载值n=(One_Circle_Distance*1000000*1000)/(variable_Running_speed_value*Subtraction)
									array_Motor_drive_send[2]=(One_Circle_Distance*5000000)/(variable_Running_speed_value*1)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,array_Motor_drive_send,1,0);
								}
								else//停止
								{
							...
							case 0x03://反转点动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									memset(array_Send_buff,0,13);
									array_Motor_drive_send[0]=GPIO_PIN_SET;
									array_Motor_drive_send[1]=((One_Circle_Distance*5000000)/(variable_Running_speed_value*1))>>8&0xff;
									array_Motor_drive_send[2]=(One_Circle_Distance*5000000)/(variable_Running_speed_value*1)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,array_Motor_drive_send,1,0);								
								}
								else//停止
...
void Motor_drive(void *argument)//电机加速启动
{
	...
	{    
		...
		if(status==osOK)
		{	
			number_Reloading=array_Motor_drive_receive[1]<<8|array_Motor_drive_receive[2];
			DO_MOTION;//PA7-电机使能
			osDelay(1);	
...
//				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
			if(number_Reloading>=1)
			{
				while(number_i>=number_Reloading&&MOTION)
				{
					__HAL_TIM_SET_AUTORELOAD(&htim8,number_i-1);//修改自动重装载值
					__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number_i/2)-1);//修改占空比
					HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
					if(25000>number_i&&number_i>=2500)//1->10mm/s------------56ms
					{
						number_i=number_i-400;
						osDelay(1);	
					}	
					else if(2500>number_i&&number_i>=1250)//10->20mm/s------------62ms
					{
						number_i=number_i-20;
						osDelay(1);	
					}
					else if(1250>number_i&&number_i>=625)//20->40mm/s------------125ms
					{
						number_i=number_i-5;
						osDelay(1);	
					}
					else if(625>number_i&&number_i>=416)//40->60mm/s------------126ms
					{
						number_i=number_i-2;
						osDelay(1);	
					}
					else if(416>number_i&&number_i>=312)//60->80mm/s------------105ms
					{
						number_i=number_i-1;
						osDelay(1);	
					}
					else if(312>number_i&&number_i>=277)//80->90mm/s------------70ms
					{
						number_i=number_i-1;
						osDelay(2);	
					}
					else//90->100mm/s------------125ms
					{
						number_i=number_i-1;
						osDelay(5);	
					}	
		...
		if(flag_Encoder_processing==1)
		{
			...
			else if(AWAY)
			{
				...
				if(flag_Zero_regression==1&&flag_Absolute_position==1)//之前回过零且接收到绝对位置
				{
					if(COROTATION)//正向移动
					{
						if(variable_Position_upload*10>=variable_Target_position)
						{
//							DO_CEASE;//PA7-电机使能
//							HAL_TIM_PWM_Stop_IT(&htim8,TIM_CHANNEL_1);//关闭PWM输出
...
void Motor_deceleration_stops(void *argument)//电机减速停止
{
  for(;;)
  {
		...
		{	
			...
			number_Reloading=(One_Circle_Distance*5000000)/(variable_Running_speed_value*1);
			while(25000>number_Reloading&&MOTION)
			{
				__HAL_TIM_SET_AUTORELOAD(&htim8,number_Reloading-1);//修改自动重装载值
				__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number_Reloading/2)-1);//修改占空比
				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）	
				if(25000>number_Reloading&&number_Reloading>=2500)//1->10mm/s------------56ms
				{
					number_Reloading=number_Reloading+400;
					osDelay(1);	
				}	
				else if(2500>number_Reloading&&number_Reloading>=1250)//10->20mm/s------------62ms
				{
					number_Reloading=number_Reloading+20;
					osDelay(1);	
				}
				else if(1250>number_Reloading&&number_Reloading>=625)//20->40mm/s------------125ms
				{
					number_Reloading=number_Reloading+5;
					osDelay(1);	
				}
				else if(625>number_Reloading&&number_Reloading>=416)//40->60mm/s------------126ms
				{
					number_Reloading=number_Reloading+2;
					osDelay(1);	
				}
				else if(416>number_Reloading&&number_Reloading>=312)//60->80mm/s------------105ms
				{
					number_Reloading=number_Reloading+1;
					osDelay(1);	
				}
				else if(312>number_Reloading&&number_Reloading>=277)//80->90mm/s------------35ms
				{
					number_Reloading=number_Reloading+1;
					osDelay(1);	
				}
				else//90->100mm/s------------50ms
				{
					number_Reloading=number_Reloading+1;
					osDelay(2);	
				}
