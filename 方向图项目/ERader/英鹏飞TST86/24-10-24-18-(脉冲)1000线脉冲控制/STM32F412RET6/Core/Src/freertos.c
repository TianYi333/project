						...
						switch(array_Drive_control[9])//判断起始地址
						{
								...
								if(array_Drive_control[10]==0xFF)//回归
								{
									if(AWAY&&CEASE)//不在原点且停止运动
									{
										...
										if(HAL_GPIO_ReadPin(IN0_GPIO_Port,IN0_Pin)==GPIO_PIN_RESET)//在头部-正向移动
										{
											...
											array_Motor_drive_send[1]=((One_Circle_Distance*1000000)/(variable_Zero_return_speed_value*1))>>8&0xff;
											array_Motor_drive_send[2]=(One_Circle_Distance*1000000)/(variable_Zero_return_speed_value*1)&0xff;
							...
							case 0x02://正转点动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									send(1,array_Send_buff,12);//向客户端发送		
									array_Motor_drive_send[0]=GPIO_PIN_RESET;
									array_Motor_drive_send[1]=((One_Circle_Distance*1000000)/(variable_Running_speed_value*1))>>8&0xff;//自动重装载值n=(One_Circle_Distance*1000000*1000)/(variable_Running_speed_value*Subtraction)
									array_Motor_drive_send[2]=(One_Circle_Distance*1000000)/(variable_Running_speed_value*1)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,array_Motor_drive_send,1,0);
								}
								else//停止
								{
							...
							case 0x03://反转点动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									send(1,array_Send_buff,12);//向客户端发送	
									array_Motor_drive_send[0]=GPIO_PIN_SET;
									array_Motor_drive_send[1]=((One_Circle_Distance*1000000)/(variable_Running_speed_value*1))>>8&0xff;
									array_Motor_drive_send[2]=(One_Circle_Distance*1000000)/(variable_Running_speed_value*1)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,array_Motor_drive_send,1,0);								
								}
								else//停止
...
//				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
			if(number_Reloading>=1)
			{
				while(number_i>=number_Reloading&&MOTION)
				{
					...
					HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
					if(5000>number_i&&number_i>=500)//1->10mm/s------------45ms
					{
						number_i=number_i-100;
						osDelay(1);	
					}	
					else if(500>number_i&&number_i>=250)//10->20mm/s------------50ms
					{
						number_i=number_i-10;
						osDelay(2);	
					}
					else if(250>number_i&&number_i>=125)//20->40mm/s------------125ms
					{
						number_i=number_i-1;
						osDelay(1);	
					}
					else if(125>number_i&&number_i>=83)//40->60mm/s------------126ms
					{
						number_i=number_i-1;
						osDelay(3);	
					}
					else if(83>number_i&&number_i>=62)//60->80mm/s------------105ms
					{
						number_i=number_i-1;
						osDelay(5);	
					}
					else if(62>number_i&&number_i>=55)//80->90mm/s------------70ms
					{
						number_i=number_i-1;
						osDelay(10);	
					}
					else//90->100mm/s------------100ms
					{
						number_i=number_i-1;
						osDelay(20);	
					}	
...
void Motor_deceleration_stops(void *argument)//电机减速停止
{
  for(;;)
  {
		...
		{	
			...
			number_Reloading=(One_Circle_Distance*1000000)/(variable_Running_speed_value*1);
			while(5000>number_Reloading&&MOTION)
			{
				__HAL_TIM_SET_AUTORELOAD(&htim8,number_Reloading-1);//修改自动重装载值
				__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number_Reloading/2)-1);//修改占空比
				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）	
				if(5000>number_Reloading&&number_Reloading>=500)//1->10mm/s
				{
					number_Reloading=number_Reloading+100;
					osDelay(1);	
				}	
				else if(500>number_Reloading&&number_Reloading>=250)//10->20mm/s
				{
					number_Reloading=number_Reloading+10;
					osDelay(1);	
				}
				else if(250>number_Reloading&&number_Reloading>=125)//20->40mm/s
				{
					number_Reloading=number_Reloading+1;
					osDelay(3);	
				}
				else if(125>number_Reloading&&number_Reloading>=62)//40->60mm/s
				{
					number_Reloading=number_Reloading+1;
					osDelay(5);	
				}
				else if(62>number_Reloading&&number_Reloading>=55)//60->80mm/s
				{
					number_Reloading=number_Reloading+1;
					osDelay(10);	
				}
