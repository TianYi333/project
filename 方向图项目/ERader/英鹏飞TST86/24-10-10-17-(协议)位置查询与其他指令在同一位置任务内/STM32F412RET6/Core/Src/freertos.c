...
void Resend_timeout(void *argument)//RS485发送接收响应超时
{
  ...
  for(;;)
  {
		...
		{
			if(count_Resend_timeout<20)
			{
				osDelay(100);	
				count_Resend_timeout++;
			}
			else if(count_Resend_timeout>=20)
			{
  ...
  for(;;)
  {
		if(flag_Resend_timeout==0)
		{
			if(MOTION)//运动状态
			{
				...
				{
//					variable_Register_address=(array_Position_Query_buffer[2]<<8|array_Position_Query_buffer[3]);
//					DO_RS485_TX;
//					HAL_UART_Transmit(&huart1,array_Position_Query_buffer,8,1);
//					DO_RS485_RX;
