...
void MX_FREERTOS_Init(void) {
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	...
	RS485_ReceiveQueueHandle = osMessageQueueNew (5, 20*sizeof(uint8_t), &RS485_ReceiveQueue_attributes);
	RS485_TransmitQueueHandle = osMessageQueueNew (5, 20*sizeof(uint8_t), &RS485_TransmitQueue_attributes);
						...
						switch(array_Drive_control[9])//判断起始地址
						{
							case 0x01://原点回归/停止
								if(array_Drive_control[10]==0xFF)//回归
								{
									...
									array_drive_TXbuffer[4]=((variable_Zero_return_speed_value/1000*12)>>8)&0xFF;//数据高位
									array_drive_TXbuffer[5]=(variable_Zero_return_speed_value/1000*12)&0xFF;//数据低位
...
void RS485_Receive_function(void *argument)//RS485接收
{
  ...
  for(;;)
  {
		...
		{
//			DO_RS485_TX;
//			HAL_UART_Transmit(&huart1,array_RS485_RXbuffer,5,1);
//			osDelay(1);
//			DO_RS485_RX;
//			HAL_UARTEx_ReceiveToIdle_DMA(&huart1,array_RS485_buffer,BUF_SIZE);//开启串口1空闲中断和DMA
//			memset(&array_RS485_RXbuffer[1] ,0, 20);
...
void RS485_Transmit_function(void *argument)//RS485发送
{
	...
	uint8_t	array_RS485_Last_TXbuffer[20];
  ...
  for(;;)
  {
		...
		{
			status=osMessageQueueGet(RS485_TransmitQueueHandle,array_RS485_TXbuffer,NULL,10);
			if(status==osOK)
			{
				if(array_RS485_TXbuffer[1]==0x03||array_RS485_TXbuffer[1]==0x04||array_RS485_TXbuffer[1]==0x06)//判断功能码
				{
					...
					memcpy(array_RS485_Last_TXbuffer, array_RS485_TXbuffer, 8);
				...
				else if(array_RS485_TXbuffer[1]==0x10)
				{
					...
					memcpy(array_RS485_Last_TXbuffer, array_RS485_TXbuffer, 9+number_Data_bytes);
					flag_Resend_timeout=1;
				}
		...
		if(flag_Resend_timeout==2)
		{	
			if(array_RS485_Last_TXbuffer[1]==0x03||array_RS485_Last_TXbuffer[1]==0x04||array_RS485_Last_TXbuffer[1]==0x06)//判断功能码
			{
				DO_RS485_TX;
				HAL_UART_Transmit(&huart1,array_RS485_Last_TXbuffer,8,1);
			}
			else if(array_RS485_Last_TXbuffer[1]==0x10)
			{
				number_Data_bytes=array_RS485_Last_TXbuffer[6];
				DO_RS485_TX;
				HAL_UART_Transmit(&huart1,array_RS485_Last_TXbuffer,9+number_Data_bytes,1);
			}
			memset(array_RS485_TXbuffer, 0, 20);
			osDelay(2);			
			DO_RS485_RX;
...
{
	uint8_t count=0;	
	uint8_t	array_Position_Query_buffer[6]={0X01,0X03,0X00,0X04,0X00,0X02};
  for(;;)
  {
		if(flag_Resend_timeout==0)
		{
