...
uint32_t variable_Zero_return_speed_value=20000;//回零速度（单位0.001mm/s）	
...
void Drive(void *argument)//控制驱动器
{
	...
	uint8_t  status=0;	
	uint32_t variable_Running_speed_value=2000;//运行速度
						...
						switch(array_Drive_control[9])//判断起始地址
						{
									...
									if(AWAY)//不在原点
									{
										if(CEASE)//判断运动状态
										{
											...
											if(HAL_GPIO_ReadPin(IN0_GPIO_Port,IN0_Pin)==GPIO_PIN_RESET)//在尾部-正向移动
											{
												...
												array_Motor_drive_send[1]=((One_Circle_Distance*1000000)/(variable_Zero_return_speed_value*4))>>8&0xff;
												array_Motor_drive_send[2]=(One_Circle_Distance*1000000)/(variable_Zero_return_speed_value*4)&0xff;
							...
							case 0x02://正转点动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									memset(array_Send_buff,0,13);	
									array_Motor_drive_send[0]=GPIO_PIN_RESET;
									array_Motor_drive_send[1]=((One_Circle_Distance*1000000)/(variable_Running_speed_value*4))>>8&0xff;//自动重装载值n=(One_Circle_Distance*1000000*1000)/(variable_Running_speed_value*Subtraction)
									array_Motor_drive_send[2]=(One_Circle_Distance*1000000)/(variable_Running_speed_value*4)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,array_Motor_drive_send,1,0);
								}
								else//停止
								{
							...
							case 0x03://反转点动/停止
								if(array_Drive_control[10]==0xFF)//开始
								{
									...
									memset(array_Send_buff,0,13);
									array_Motor_drive_send[0]=GPIO_PIN_SET;
									array_Motor_drive_send[1]=((One_Circle_Distance*1000000)/(variable_Running_speed_value*4))>>8&0xff;
									array_Motor_drive_send[2]=(One_Circle_Distance*1000000)/(variable_Running_speed_value*4)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,array_Motor_drive_send,1,0);								
								}
								else//停止
