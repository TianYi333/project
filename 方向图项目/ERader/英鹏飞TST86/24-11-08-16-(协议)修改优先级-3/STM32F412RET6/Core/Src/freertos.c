...
//																	(variable_Position*1000>=variable_Target_position&&variable_Position*1000<=variable_Target_position*1.1)))
									else if(CEASE&&((variable_Target_position>=variable_Position*1000&&variable_Target_position-variable_Position*1000<1000)||(variable_Position*1000>=variable_Target_position&&variable_Position*1000-variable_Target_position<1000)))
									{
										array_Send_buff[8]=number_Data_bytes;//接下来数据的字节长度
...
void RS485_Receive_function(void *argument)//RS485接收
{
  ...
  {
		osDelay(1);
		status=osMessageQueueGet(RS485_ReceiveQueueHandle,array_RS485_RXbuffer,NULL,50);
		if(status==osOK)
		{
...
void RS485_Transmit_function(void *argument)//RS485发送
{
  ...
  {
		osDelay(1);
		status=osMessageQueueGet(RS485_TransmitQueueHandle,array_RS485_TXbuffer,NULL,50);
		if(status==osOK)
		{
...
//			HAL_UARTEx_ReceiveToIdle_DMA(&huart1,array_RS485_buffer,BUF_SIZE);//开启串口1空闲中断和DMA,
		...
		{
			array_RS485_TXbuffer[0]=0x01;
			array_RS485_TXbuffer[1]=0x03;
			array_RS485_TXbuffer[2]=0x00;
			array_RS485_TXbuffer[3]=0x04;
			array_RS485_TXbuffer[4]=0x00;
			array_RS485_TXbuffer[5]=0x02;
			array_RS485_TXbuffer[6]=0x85;
			array_RS485_TXbuffer[7]=0xCA;
			variable_Register_address=0x0004;
			DO_RS485_TX;
			HAL_UART_Transmit(&huart1,array_RS485_TXbuffer,8,10);
			DO_RS485_RX;		
			memset(array_RS485_TXbuffer, 0, 20);			
  ...
  for(;;)
  {
		...
		if(flag_First_power_on==0)
		{
			if(CEASE)
			{
				...
				array_Zeroing[4]=0x9E;
				array_Zeroing[5]=0x00;//120转/分
				array_Zeroing[6]=0x31;
