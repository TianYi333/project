					...
					if(Drive_control_array[8]==0x20)//判断起始地址
					{
						switch(Drive_control_array[9])//判断起始地址
						{
							...
							case 0x01://原点回归/停止
								...
								send_buff[5]=0X05;//数据长度
								send_buff[8]=0X01;//接下来数据的字节长度
								if(Sports_status_flag==1&&Position_flag==0)//回零中
								{
									send_buff[9]=0X01;
								}
								else if(Sports_status_flag==0&&Position_flag==1)//回零
								{
									send_buff[9]=0X00;
								}													
								send(1,send_buff,10);//向客户端发送	
						...
						switch(Drive_control_array[9])//判断起始地址
						{
							case 0x01://原点回归/停止
									...
									if(Position_flag==0)//不在原点
									{
										if(Sports_status_flag==0)//判断运动状态
										{
											...
											if(HAL_GPIO_ReadPin(IN0_GPIO_Port,IN0_Pin)==GPIO_PIN_RESET)//在尾部-正向移动
											{
												...
												Motor_drive_send[1]=((One_Circle_Distance*1000000)/(zero_return_speed_value*4))>>8&0xff;
												Motor_drive_send[2]=(One_Circle_Distance*1000000)/(zero_return_speed_value*4)&0xff;
								...
								else if(Drive_control_array[10]==0x00)//停止
								{
									...
									send(1,send_buff,12);//向客户端发送	
									memset(send_buff,0,13);	
									HAL_GPIO_WritePin(TIM8_CH1N_GPIO_Port,TIM8_CH1N_Pin,GPIO_PIN_RESET);//PA7-电机使能正输入(GPIO_PIN_SET:开始，GPIO_PIN_RESET:停止）
							...
							case 0x02://正转点动/停止
								if(Drive_control_array[10]==0xFF)//开始
								{
									...
									Position_flag=0;
									Motor_drive_send[0]=GPIO_PIN_RESET;
									Motor_drive_send[1]=((One_Circle_Distance*1000000)/(running_speed_value*4))>>8&0xff;//自动重装载值n=(One_Circle_Distance*1000000*1000)/(running_speed_value*Subtraction)
									Motor_drive_send[2]=(One_Circle_Distance*1000000)/(running_speed_value*4)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,Motor_drive_send,1,0);
								}
								else//停止
								{
							...
							case 0x03://反转点动/停止
								if(Drive_control_array[10]==0xFF)//开始
								{
									...
									Position_flag=0;
									Motor_drive_send[0]=GPIO_PIN_SET;
									Motor_drive_send[1]=((One_Circle_Distance*1000000)/(running_speed_value*4))>>8&0xff;
									Motor_drive_send[2]=(One_Circle_Distance*1000000)/(running_speed_value*4)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,Motor_drive_send,1,0);								
								}
								else//停止
							...
							case 0x05://绝对运动/停止
								if(Drive_control_array[10]==0xFF)//开始
								{
									...
									if(Sports_status_flag==0&&Extreme_position_flag==0)
									{
										...
										else if(Position_upload*10>Target_Position&&Position_flag==0)//当前位置大于目标位置且不在原点-反向移动
										{
											Motor_drive_send[0]=GPIO_PIN_SET;
											Motor_drive_send[1]=((One_Circle_Distance*1000000)/(running_speed_value*4))>>8&0xff;
											Motor_drive_send[2]=(One_Circle_Distance*1000000)/(running_speed_value*4)&0xff;
...
{
	uint8_t status=0;	
	uint16_t number_i=2500;
	uint16_t number;
	uint8_t Motor_drive_receive[3];//电机驱动数组
	for(;;)
	{    
		status=osMessageQueueGet(Motor_driveQueueHandle,Motor_drive_receive,NULL,0);
		if(status==osOK)
		{	
			if(Sports_status_flag==1)
			{
				number=Motor_drive_receive[1]<<8|Motor_drive_receive[2];
				HAL_GPIO_WritePin(TIM8_CH1N_GPIO_Port,TIM8_CH1N_Pin,GPIO_PIN_SET);//PA7-电机使能正输入(GPIO_PIN_SET:开始，GPIO_PIN_RESET:停止）
				osDelay(1);	
...
//				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
				if(number>=10)
				{
					while(number_i>=number&&HAL_GPIO_ReadPin(TIM8_CH1N_GPIO_Port,TIM8_CH1N_Pin)==GPIO_PIN_SET)
					{
						__HAL_TIM_SET_AUTORELOAD(&htim8,number_i-1);//修改自动重装载值
						__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number_i/2)-1);//修改占空比
						HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
						if(2500>number_i&&number_i>=250)
						{
							number_i=number_i-50;
							osDelay(1);	
						}	
						else if(250>number_i&&number_i>=125)
						{
							number_i=number_i-5;
							osDelay(2);	
						}
