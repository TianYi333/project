  ...
  for(;;)
  {
    osDelay(10);	
		if(IC_DONE_FLAG == 1)  // 如果完成一次高电平捕获
		{
			...
			osMessagePut(Pulse_Reception_QueueHandle,(uint32_t)&Pulse_Cycle_array,0);
		}
  }
...
void Screen(void *argument)
{
  ...
  for(;;)
  {
		status=osMessagePut(Pulse_Reception_QueueHandle,(uint32_t)&Cycle_Send_array,5000);
		if(status!=osOK)
		{		
			osDelay(500);
		...
		{
			TJCPrintf("tm0.en=\"%d\"",1);//关闭屏幕数据复位定时器
