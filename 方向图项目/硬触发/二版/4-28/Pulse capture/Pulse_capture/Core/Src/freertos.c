...
void Screen(void *argument)
{
  ...
  {
    osDelay(1);
		if(START_FLAG==1&&IC_DONE_FLAG==1)//判断是否接受到开始捕获命令且完成一次高电平捕获
		{
			...
			{	
				if(Pulse_frame_rate<Pulse_standard_frame_rate)//判断帧数
				{
					...
					TJCPrintf("gather.t12.txt=\"%d\"",Pulse_frame_rate);	//显示脉冲帧数
				}
				else
				{
					Pulse_frame_rate=0;
			...
			else
			{
				if(Pulse_Cycle_Send_array[0]==0x55&&Pulse_Cycle_Send_array[5]==0xff)
				{
					...
					if((One_cycle_angle-Standard_Cycle_angle>Error_Criteria_angle)||(Standard_Cycle_angle-One_cycle_angle>Error_Criteria_angle)) //TIME不等于0，且周期误差大于误差标准
					{	
