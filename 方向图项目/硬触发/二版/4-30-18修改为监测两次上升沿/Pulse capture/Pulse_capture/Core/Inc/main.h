...
extern uint32_t IC_TIMES_1;  		// 捕获次数，单位1ms
extern uint32_t IC_TIMES_2;  		// 捕获低电平次数，单位1ms
extern uint8_t IC_END_FLAG;	// 捕获结束标志位
extern uint8_t IC_START_FLAG;  // 捕获开始标志，1：已捕获到高电平；0：还没有捕获到高电平
extern uint8_t IC_DONE_FLAG;  	// 捕获完成标志，1：已完成一次高电平捕获
extern uint8_t IC_DOWN_DONE_FLAG;// 低电压捕获完成标志，1：已完成一次低电平捕获
extern uint16_t IC_VALUE_1;  		// 输入捕获的捕获值
extern uint16_t IC_VALUE_2;  		// 下降沿输入捕获的捕获值
extern uint16_t IC_DOWN_START_FLAG;  		// 下降沿输入捕获开始标志，1：开始；0：还没有开始
