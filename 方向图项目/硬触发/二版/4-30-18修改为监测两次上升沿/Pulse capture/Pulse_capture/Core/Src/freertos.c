...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
uint32_t IC_TIMES_1;  		// 捕获次数，单位1ms
uint32_t IC_TIMES_2;  		// 捕获低电平次数，单位1ms
...
uint16_t IC_VALUE_1;  		// 输入捕获的捕获值
uint16_t IC_VALUE_2;  		// 下降沿输入捕获的捕获值
  ...
  for(;;)
  {
    osDelay(1);
		if(IC_DONE_FLAG == 1)	// 如果完成一次高电平捕获
		{
			...
			Pulse_Cycle = IC_TIMES_1* 1000;	//脉冲时间为捕获次数 * 1000us
			Pulse_Cycle += IC_VALUE_1;	 // 加上捕获时间（小于1ms的部分)
		...
		else if(IC_DONE_FLAG == 2)	// 如果完成一次高电平捕获
		{
			IC_DONE_FLAG = 0;	//标志清零
			if(IC_END_FLAG==1)
			{
			...
			Pulse_Cycle = IC_TIMES_2* 1000;	//脉冲时间为捕获次数 * 1000us
			Pulse_Cycle += IC_VALUE_2;	 // 加上捕获时间（小于1ms的部分)
			if(Pulse_Cycle>=Pulse_Filtering)	//滤波
			{
				Pulse_Cycle_array[0]=0x55;
				Pulse_Cycle_array[1]=0xB2;
